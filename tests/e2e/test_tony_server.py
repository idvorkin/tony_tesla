...
def test_send_text():
    ...
    if "text message sent" in result["results"][0]["result"].lower():
        ...
        assert ("missing twilio configuration" in result["results"][0]["result"].lower() or
                "failed to send message" in result["results"][0]["result"].lower())
...
def test_send_text_ifttt():
    ...
    try:
        ...
        if "text message sent via ifttt" in result["results"][0]["result"].lower():
            ...
            assert "error" in result["results"][0]["result"].lower()
            assert ("missing ifttt configuration" in result["results"][0]["result"].lower() or
                    "failed to send webhook request" in result["results"][0]["result"].lower())
    except requests.exceptions.RequestException as e:
