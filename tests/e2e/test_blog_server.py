...
def test_random_blog_e2e(auth_headers, base_params):
    ...
    try:
        response = make_request(
            f"{BLOG_SERVER_BASE}/random_blog", base_params, auth_headers
        )
...
def test_blog_info_e2e(auth_headers, base_params):
    ...
    try:
        response = make_request(
            f"{BLOG_SERVER_BASE}/blog_info", base_params, auth_headers
        )
        result = response.json()
        ...
        assert len(result_dict) > 100, (
            f"Expected more than 100 blog posts, but got {len(result_dict)}"
        )
...
def test_read_blog_post_e2e(auth_headers, base_params):
    ...
    base_params["message"]["toolCalls"][0]["function"].update(
        {"name": "read_blog_post", "arguments": {"path": "_d/vim_tips.md"}}
    )

    try:
        response = make_request(
            f"{BLOG_SERVER_BASE}/read_blog_post", base_params, auth_headers
        )
        result = response.json()
        ...
        assert len(result_dict["content"]) > 0, "Content should not be empty"
        assert result_dict["markdown_path"] == "_d/vim_tips.md", (
            "Incorrect markdown path from URL"
        )
...
def test_random_blog_url_e2e(auth_headers, base_params):
    ...
    try:
        response = make_request(
            f"{BLOG_SERVER_BASE}/random_blog_url", base_params, auth_headers
        )
        result = response.json()
...
def test_blog_search_e2e(auth_headers, base_params):
    ...
    base_params["message"]["toolCalls"][0]["function"].update(
        {"name": "blog_search", "arguments": {"query": "meditation"}}
    )

    try:
        response = make_request(
            f"{BLOG_SERVER_BASE}/blog_search", base_params, auth_headers
        )
        result = response.json()
        ...
        assert any(
            "meditation" in result["content"].lower() for result in search_results
