...
class TripUpdate(BaseModel):
    ...
    class StopTimeUpdate(BaseModel):
        stop_id: str
        arrival: TripUpdate.Arrival | None = None
        departure: TripUpdate.Departure | None = None
...
def stops_for_route(route: str):
    ...
    for trip_update in trip_updates:
        ...
        for trip in trips:
            print("# TRIP: ", trip.trip.trip_id, trip.trip.direction_id)
            for stop_time_update in trip.stop_time_update:
                stop_id = stop_time_update.stop_id
                stop = get_stops()[stop_id]
                stop_name = stop.stop_name
                arrival_time = stop_time_update.arrival.time
                arrival_time_pst = datetime.fromtimestamp(arrival_time).astimezone(
